...
- `~/.zcode/cli/plugins/data/data-analytics@zcode-community/user-context.md` owns durable user-approved source-routing preferences and semantic-layer registry pointers.
- `~/.zcode/cli/plugins/data/data-analytics@zcode-community/onboarding-state.json` owns compact onboarding progress, connector-confirmation labels, semantic-layer setup and refresh state, hero prompt progress, and environment conflict notes. Do not persist raw discovery or inventory payloads there.
