...
Use this reference when `~/.zcode/cli/plugins/data/data-analytics@zcode-community/user-context.md` is missing or unreadable, when the user asks what Data Analytics can do, when the user asks to set up or customize Data Analytics, or when `onboarding-state.json` says onboarding is active and a nudge is due.
