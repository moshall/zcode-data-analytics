...
def default_state_root() -> Path:
    """Resolve the Data Analytics state root for ZCode."""
    env_dir = os.environ.get("ZCODE_DATA_ANALYTICS_STATE_DIR")
    if env_dir:
        return Path(env_dir).expanduser()
    return Path.home() / ".zcode" / "cli" / "plugins" / "data" / "data-analytics@zcode-community"


def state_dir_from_args(args: argparse.Namespace) -> Path:
    if args.state_dir:
        return args.state_dir.expanduser()
    return args.state_root.expanduser() if args.state_root else default_state_root()
...
def load_app_connector_ids() -> dict[str, str]:
    # ZCode port: Codex connector layer (.app.json) is not available.
    return {}
