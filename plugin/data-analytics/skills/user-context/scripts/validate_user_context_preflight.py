...
USER_CONTEXT_MANDATORY_GATE_PHRASES = (
    ...
    "reports read status for `~/.zcode/cli/plugins/data/data-analytics@zcode-community/user-context.md`",
)
...
def main() -> int:
    ...
    required_paths = (
        ...
        skills_root / "user-context/agents/openai.yaml",
    ...
    # ZCode port: the Codex connector layer (.app.json) is intentionally absent.
    # Only validate it when present; otherwise skip connector cross-checks.
            ...
            for category_id, metadata in categories.items():
                ...
                if not isinstance(preferred_plugins, list) or not preferred_plugins:
                    ...
                    for preferred_plugin in preferred_plugins:
                        if manifest_app_aliases and normalize_route_name(
                            preferred_plugin
                        ) not in manifest_app_aliases:
                            failures.append(
