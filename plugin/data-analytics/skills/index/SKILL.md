...
- Default to static file outputs in ZCode: HTML reports with embedded PNG charts, PDF, spreadsheets, notebooks, or slides. Charts must be static PNG images generated from a plotting library (matplotlib/Seaborn) and embedded in the report file.
...
- Once the analysis commits to a source table in an inline or dashboard route, expose a small deterministic preview when safe through a CSV/HTML snippet in the selected surface's normal preview mechanism.
...
- Choose a static file delivery mode when the user asks for HTML, file/blob sharing, Docs/Slides conversion, offline portability, or when an interactive rendering surface is unavailable, unsafe, or over bounds.
...
Use $build-dashboard for analytical dashboards, scorecards, monitoring pages, BI views, Streamlit dashboards, BI platform dashboards, and dashboard QA.

### build-report

Use $build-report to build exactly one durable report surface selected for the user request, such as an HTML report with Seaborn-generated PNG charts.
